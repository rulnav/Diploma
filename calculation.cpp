...
int instance (int subp, uint16_t EepromData[], uint16_t RamData[], uint16_t resolution_register)
{
  FILE *fp;
  int j = 0;
  ...
  fprintf(fp,"Subpage is %d \nThis is Eeprom.\n",subp);
   for (j = 0; j<832;j++)
   {
       ...
       {
			fprintf(fp,"'000%X,",EepromData[j]);
	   }
	   else if (EepromData[j]< 0x100)
	   {
		    fprintf(fp,"'00%X,",EepromData[j]);
	   }
	   else if (EepromData[j]< 0x1000)
	   {
		    fprintf(fp,"'0%X,",EepromData[j]);
	   }
	   else
	   {
		    fprintf(fp,"'%X,",EepromData[j]);
	   }
   }
  fprintf(fp,"\nThis is Ram.\n");
   /*for (j = 0;j<447;j++)
